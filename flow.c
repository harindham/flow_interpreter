...
int validate_component(Flow *flow, const char *name, char stack[][64], int stack_count, char visited[][64], int *visited_count)
{
    ...
    for (int i = 0; i < stack_count; i++)
    {
        if (strncmp(stack[i], name, 64) == 0)
        {
            ...
            for (int j = i; j < stack_count; j++)
            {
                fprintf(stderr, "%s -> ", stack[j]);
            }
            fprintf(stderr, "%s\n", name);
            return 1;
        }
    ...
    if (p)
    {
        if (validate_component(flow, p->from, stack, stack_count + 1, visited, visited_count))
            return 1;
        if (validate_component(flow, p->to, stack, stack_count + 1, visited, visited_count))
            return 1;
    }

    Concat *c = find_concat(flow, name);
    if (c)
    {
        for (int i = 0; i < c->parts; i++)
        {
            if (validate_component(flow, c->partnames[i], stack, stack_count + 1, visited, visited_count))
                return 1;
        }
    }

    StderrRedirect *e = find_stderr(flow, name);
    if (e)
    {
        if (validate_component(flow, e->from, stack, stack_count + 1, visited, visited_count))
            return 1;
    }
...
void validate_flow(Flow *flow)
{
    char stack[MAX_CALL_STACK][64] = {0};
...
int main(int argc, char *argv[])
{
    ...
    // VALIDATE FOR CYCLES BEFORE EXECUTION
    validate_flow(&flow);
